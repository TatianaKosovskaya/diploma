...
header = st.container()
with header:
    st.header("Recommendation System for Staff")
...
if yes and not no:
    ...
    if type_of_rs == 'Collaborative Filtering with Memory Limit':
        st.write("**Please select one or two products used by the client.**")
    ...
    if click:
        if type_of_rs == 'Collaborative Filtering':
            ...
            if (sum(arr1 + arr2) > 0) and (sum(arr1 + arr2) != len(product_list)):
        ...
        if type_of_rs == 'Collaborative Filtering with Memory Limit':
            ...
            if (sum(arr1 + arr2) < 3) and (sum(arr1 + arr2) != 0):
                with st.spinner('Due to the calculation of the machine learning model you have to wait a few seconds for the result.'):
                    ...
                    d = get_recommendation(9999999,model=sim_user)
                    d = d['service_name']
                    d = d[:5]
                    d = list(d)
                    st.subheader('Recommended products:')
    ...
    if click:
        if type_of_rs == 'Collaborative Filtering Based on Item Memory':
            ...
            if (sum(arr1 + arr2) > 0) and (sum(arr1 + arr2) != len(product_list)):
                with st.spinner('Due to the calculation of the machine learning model you have to wait a few seconds for the result.'):
                    ...
                    d = get_recommendation(9999999,sim_item)
                    d = d['service_name']
                    d = d[:5]
                    d = list(d)
                    st.subheader('Recommended products:')
    ...
    if click:
        if type_of_rs == 'Collaborative Filtering':
                with st.spinner('Due to the calculation of the machine learning model you have to wait a few seconds for the result.'):
                    ...
                    d = get_recommendation(number_id,svd)
                    d = d['service_name']
                    d = d[:5]
                    d = list(d)
                    st.subheader('Recommended products:')
    ...
    if click:
        if type_of_rs == 'Collaborative Filtering with Memory Limit':
                with st.spinner('Due to the calculation of the machine learning model you have to wait a few seconds for the result.'):
                    ...
                    d = get_recommendation(number_id,model=sim_user)
                    d = d['service_name']
                    d = d[:5]
                    d = list(d)
                    st.subheader('Recommended products:')
    ...
    if click:
        if type_of_rs == 'Collaborative Filtering Based on Item Memory':
                with st.spinner('Due to the calculation of the machine learning model you have to wait a few seconds for the result.'):
                    ...
                    d = get_recommendation(number_id,sim_item)
                    d = d['service_name']
                    d = d[:5]
                    d = list(d)
                    st.subheader('Recommended products:')
